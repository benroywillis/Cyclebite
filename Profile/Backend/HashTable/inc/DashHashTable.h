...
// size of the array for each hash index
#define TUPLE_SIZE 15

// This number is only allowed to be 1
// It used to be used to show what happens when the profile increases the dimension of the transition table
// But it turns out that doesn't help our analysis, and makes transformations and processing more complicated
...
extern "C"
{
    ...
    typedef struct callerTuple
    {
        ...
        uint32_t blocks[MARKOV_ORDER + 1];
        // in order to preserve the order of function calls within a basic block, this integer records which function call in the basic block is the one this entry represents
        uint32_t position;
    } __TA_callerTuple;
